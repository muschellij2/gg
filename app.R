...
Sn/√n is a continuous distribution and lie roughly between -3 and 3. By the central limit theorem we have Sn/√n converges in distribution to a standard normal random variable.')
  ...
  output$plot2=renderPlot({
    ...
    un = unique(data$n)
    x = data$sn_n
    print(paste0("N_Sn_n=", length(x)))
    print(paste0("N_real_Sn_n=", sum(!is.na(x))))
    print(range(x))
    print(hist_lims())
    print(length(nclass.Sturges(x)))
  ...
  output$plot3=renderPlot({
    ...
    un = unique(data$n)
    x = data$sn_sqrtn
    print(paste0("N_Sn_sqrtn=", length(x)))
    print(paste0("N_real_Sn_sqrtn=", sum(!is.na(x))))
    print(range(x))
    print(hist_lims())
    print(length(nclass.Sturges(x)))
  ...
  output$plot4=renderPlot({
    ...
    un = unique(data$n)
    x = data$sn_loglog
    print(paste0("N_loglog=", length(x)))
    print(paste0("N_real_loglog=", sum(!is.na(x))))
    print(range(x))
    print(hist_lims())
    print(length(nclass.Sturges(x)))
    ...
    myhist = run_hist(
      ...
      xlim = hist_lims(),
      xlab='Sn/√{nloglog(n)}',ylab='Frequency',cex.lab=2, 
      cex.axis=2, cex.main=2, cex.sub=2,col='lightblue')
    print(length(myhist$breaks))
    plot(myhist)
  })
  observeEvent(input$go,{
